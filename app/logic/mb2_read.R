...
#'
#' @param file_path correspond to the file location
#' @return a list containing all the extracted information from the .mb2 file
#' @author Romain Garreau
#' @export 

read_mb2 <- function(file_path) {
  ...
  # Extract Drug, MIC, and number of dose/concentrations
  parts <- strsplit(lines[16 + weight_number], "\\s+")[[1]]
  drug_name <- parts[1]

  # Extract MIC value if present
  mic_value <- 0
  if (length(parts) > 1) {
    # Try to extract MIC from the 2nd part (should be a numeric value)
    tryCatch(
      {
        mic_value <- as.numeric(parts[2])
      },
      error = function(e) {
        mic_value <- 0 # Default to 0 if not parseable
      }
    )
  }
  ...
  return(list(
    ...
    "dose_df" = dose_df,
    "level_df" = level_df,
    "indication" = NA, # Added for future use
    "identified_bacteria" = NA # Added for future use
  ))
}
