box::use(
  ...
  shiny[column, conditionalPanel, dateInput, fluidRow, icon, moduleServer, NS, numericInput, reactiveValues, selectInput, tabPanel, tagList, tags],
  shinyTime[timeInput],
    ...
    fluidRow(
      	...
      	box( # This section handle all covariate data
      	  ...
      	  fluidRow(
			  ...
			  actionButton(ns("renal_formula_calculator"), translator$t("Calculateur rénal"), style = "background-color: #3d9970; color: white;")
      	    )
      	   )
      	),
      	box( # This section handle all administration related data
      	  ...
      	  width = 12,
      	  solidHeader = TRUE,
      	  selectInput(ns("administration_route"), translator$t("Voie d'administration"), choices = c("IV", "IM", "PO", "CI"), selected = "IV"),
      	  conditionalPanel(
      	    ...
      	    fluidRow(
      	      column(width = 6, dateInput(ns("end_date_CI"), translator$t("Date de fin de perfusion"), Sys.Date(), format = "yyyy-mm-dd", language = "fr")),
